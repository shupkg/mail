...
find ./tmp -name '*.go' -exec sed -i '' 's/SMTPServer/Server/g' {} \;
find ./tmp -name '*.go' -exec sed -i '' 's/SMTPClient/Client/g' {} \;
find ./tmp -name '*.go' -exec sed -i '' 's/NewMSG/NewEmail/g' {} \;
find ./tmp -name '*.go' -exec sed -i '' 's/NewClient/New/g' {} \;

find ./tmp -name '*.go' -exec sed -i '' "s?${oldPkg}?${newPkg}?g;s?${oldName}?${newName}?g" {} \;
...
wget https://raw.githubusercontent.com/alex1989hu/go-simple-mail/ci/add-golangci-lint/.golangci.yml

go mod init github.com/shupkg/mail
