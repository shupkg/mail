...
func (server *Server) GetEncryptionType() Encryption {
...
func (email *Email) SendEnvelopeFrom(from string, client *Client) error {
	if email.Error != nil {
		return email.Error
	}
...
func SendMessage(from string, recipients []string, msg string, client *Client) error {
	if from == "" {
